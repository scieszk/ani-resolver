...
1. Call `ani_resolver_provider_list` when provider capabilities or readiness are unknown. Select sources based on their declared capabilities; do not assume TMDB supports character search.
...
5. For a character clue, call `ani_resolver_resolve_character`. When the work is known, pass a `bangumi:<id>` work constraint or call `ani_resolver_work_characters`; this is usually stronger than global trait text search.
6. Call `ani_resolver_entity_get` when an explicit external ID needs upstream verification.
...
- Bangumi Archive appearance matching is limited to text present in the archive; missing text is missing evidence.
