...
1. Run `ani-resolver providers list --json` when available providers are unknown. Choose providers whose declared capabilities fit the request; do not assume every provider supports characters or TMDB IDs.
...
7. For a character name or clue, use character-capable providers. When a work is known, pass `--work <id>` or inspect `work characters`; this usually narrows vague traits more effectively than global text search. Compare returned facts and summaries yourself. The CLI does not generate follow-up questions.
